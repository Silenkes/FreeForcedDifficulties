...
#include "CPP_IncreasingWeaponComponent.h"
...
void UCPP_IncreasingWeaponComponent::Fire()
{
...
void UCPP_IncreasingWeaponComponent::BeginPlay()
{
	Super::BeginPlay();
...
// Single linetrace for fire
void UCPP_IncreasingWeaponComponent::LineTraceByChannel()
{
	...
	if (bHit)
	{
		...
		{
			ShapeScaleChangingInterface->IncreaseSize();
		}
	}

		if (FireSound)
		{
			UGameplayStatics::PlaySoundAtLocation(this, FireSound, GetSocketLocation("Muzzle"), 0.7f);
		}
		...
		if (FireParticle)
		{
			UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), FireParticle, FTransform(GetSocketRotation("Muzzle"), GetSocketLocation("Muzzle"), FVector(0.25f, 0.25f, 0.25f)), true);
		}
}

void UCPP_IncreasingWeaponComponent::DropWeapon()
{
...
void UCPP_IncreasingWeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{	
	if (!OwnerCharacter)
		return;
